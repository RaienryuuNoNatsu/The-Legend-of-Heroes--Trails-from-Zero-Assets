...
You can download the archive from the [Releases](https://github.com/RaienryuuNoNatsu/The-Legend-of-Heroes--Trails-from-Zero-Assets/releases) page. At this point, the content as is will be in PNG, which doesn't work with the game as it looks for ITPs instead. The reason I'm leaving them as PNGs is so you can choose what to keep and what to drop. Once you're done deciding, just hit the **Convert.bat** which will simply change the files extension. Now, drag & drop the entire `data` folder in the main game's directory.